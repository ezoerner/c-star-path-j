...
*/
    ...
    public static void storeKeyValuePairs(final String rowKey,
                                          final String columnFamilyName,
                                          final Map<String, PropertyValue<?>> keyValuePairs,
    ...
    public static void storeKeyValuePairs(final String rowKey,
        ...
        template.write(keyspaceName, new KeyspaceCallback<Void>() {
            public Void execute(KeyspaceService keyspace) {
                ...
                for (Map.Entry<String, PropertyValue<?>> keyValue : keyValuePairs.entrySet()) {
     ...
     */
    public static Map<String, PropertyValue<?>> getAllKeyValuePairs(final String rowKey,
                                                                    final String columnFamilyName,
                                                                    CassandraTemplate template,
                                                                    String keyspaceName) {
            ...
            public Map<String, PropertyValue<?>> execute(KeyspaceService keyspace) {
                ...
                for (Column c : list) {
                    props.put(string(c.getName()),
                              PropertyValueFactory.get().createPropertyValue(c.getValue()));
                }
                return props;
            }
        });
     ...
     * @return the value
     */
    public static PropertyValue<?> getOneValue(final String rowKey,
                                               final String columnFamilyName,
                                               final String columnName,
                                               CassandraTemplate template,
                                               String keyspaceName) {
        ...
        return template.read(keyspaceName, new KeyspaceCallback<PropertyValue>() {
            public PropertyValue execute(KeyspaceService keyspace) {
                ...
                try {
                    ...
                    return PropertyValueFactory.get().createPropertyValue(column.getValue());
    ...
    public static void storeKeyValuePairsToBatchMutation(String rowKey,
                                                         String columnFamilyName,
                                                         Map<String, PropertyValue<?>> keyValuePairs,
                                                         CassandraTemplate template,
                                                         BatchMutation<String> batchMutation) {
    ...
    /**
     * Use the properties from the subcolumns of a SuperColumn to create a Map<String, PropertyValue>.
     *
     * @param sc           the super column to get the properties from
     * @return the newly created Map<String, PropertyValue>
     */
    public static Map<String, PropertyValue<?>> createPropertiesFromSuperColumn(SuperColumn sc) {
        Map<String, PropertyValue<?>> properties = new HashMap<String, PropertyValue<?>>();
        for (Column column : sc.getColumns()) {
            properties.put(string(column.getName()), PropertyValueFactory.get().createPropertyValue(column.getValue()));
        }
        return properties;
    }
    ...
    private static void addPropertiesToBatchMutation(String rowKey,
        ...
        for (Map.Entry<String, PropertyValue<?>> entry : properties.entrySet()) {
            ...
            column.setValue(entry.getValue().toBytes());
            column.setTimestamp(timestamp);
            batchMutation.addInsertion(rowKey, Collections.singletonList(columnFamilyName), column);
        }
    }
